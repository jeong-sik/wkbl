...
let player_row ?(show_player_id=false) ?(team_cell_class="px-3 py-2") ?(include_team=true) (rank: int) (p: player_aggregate) =
  ...
  Printf.sprintf
    {html|<tr class="group border-b border-slate-700/50 hover:bg-slate-800/50 transition-colors">
      ...
      %s
      <td class="px-3 py-2 text-right text-slate-400 whitespace-nowrap">%d</td>
      %s%s%s%s%s%s%s%s
    </tr>|html}
    rank
    ...
    (stat_total_cell ~extra_classes:"hidden md:table-cell" p.avg_steals p.total_steals)
    (stat_total_cell ~extra_classes:"hidden md:table-cell" p.avg_blocks p.total_blocks)
    (stat_total_cell ~extra_classes:"hidden md:table-cell" p.avg_turnovers p.total_turnovers)
...
let players_table (players: player_aggregate list) =
  ...
  Printf.sprintf
    ...
    <table class="min-w-[760px] sm:min-w-[920px] w-full text-xs sm:text-sm font-mono tabular-nums table-auto">
      <thead class="bg-slate-800/80 sticky top-0 text-slate-400 text-[10px] sm:text-xs uppercase tracking-wider whitespace-nowrap">
        <tr>
          ...
          <th class="px-3 py-3 text-left w-[260px]">Player</th>
          <th class="px-3 py-3 text-left w-[160px]">Team</th>
          <th class="px-3 py-3 text-right w-[60px]">GP</th>
          ...
          <th class="px-3 py-3 text-right w-[72px] cursor-pointer hover:text-orange-400" title="MG: 팀 득실마진(출전시간 가중 평균)" hx-get="/players/table?sort=mg" hx-target="#players-table" hx-swap="innerHTML" hx-include="#players-filter">MG</th>
          ...
          <th class="px-3 py-3 text-right w-[72px] cursor-pointer hover:text-orange-400" hx-get="/players/table?sort=ast" hx-target="#players-table" hx-swap="innerHTML" hx-include="#players-filter">AST</th>
          <th class="px-3 py-3 text-right w-[72px] hidden md:table-cell">STL</th>
          <th class="px-3 py-3 text-right w-[72px] hidden md:table-cell">BLK</th>
          <th class="px-3 py-3 text-right w-[72px] hidden md:table-cell">TO</th>
...
<head>
  ...
  <script src="/static/js/loading-indicator.js?v=%s" defer data-cfasync="false"></script>
...
<body class="bg-[#0b0e14] text-slate-200 font-sans antialiased min-h-screen">
  <div id="global-loading" class="global-loading" aria-hidden="true"></div>
  <header class="sticky top-0 z-50 bg-slate-900/95 backdrop-blur border-b border-slate-800 px-4 sm:px-6 py-3 sm:py-4">
    <div class="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <nav class="flex flex-wrap items-center gap-3 text-xs sm:text-sm sm:flex-nowrap sm:gap-x-4 sm:gap-y-2 sm:justify-end">
...
let standings_table ~season (standings : team_standing list) =
  let rows =
    ...
    |> List.mapi (fun i (s : team_standing) ->
        ...
        in
        Printf.sprintf
          {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-4 py-3 text-slate-500 font-mono text-sm">%d</td><td class="px-4 py-3 font-bold text-white"><div class="flex items-center gap-2 min-w-0">%s<a href="%s" class="player-name min-w-0 hover:text-orange-400 transition-colors">%s</a></div></td><td class="px-4 py-3 text-right text-slate-300">%d</td><td class="px-4 py-3 text-right text-slate-300">%d</td><td class="px-4 py-3 text-right text-slate-300">%d</td><td class="px-4 py-3 text-right text-orange-400 font-bold">%s</td><td class="px-4 py-3 text-right text-slate-400 hidden sm:table-cell">%s</td><td class="px-4 py-3 text-right text-slate-300 font-mono hidden md:table-cell">%.1f</td><td class="px-4 py-3 text-right text-slate-300 font-mono hidden md:table-cell">%.1f</td><td class="px-4 py-3 text-right %s font-mono font-bold hidden md:table-cell">%.1f</td></tr>|html}
  ...
  in
  Printf.sprintf
    {html|<div class="bg-slate-900 rounded-lg border border-slate-800 overflow-x-auto overflow-y-hidden shadow-2xl"><table class="min-w-[520px] sm:min-w-[760px] w-full text-xs sm:text-sm font-mono tabular-nums"><thead class="bg-slate-800/80 text-slate-400 text-[10px] sm:text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-4 py-3 text-left w-10">Rk</th><th class="px-4 py-3 text-left font-sans">Team</th><th class="px-4 py-3 text-right">GP</th><th class="px-4 py-3 text-right">W</th><th class="px-4 py-3 text-right">L</th><th class="px-4 py-3 text-right">PCT</th><th class="px-4 py-3 text-right hidden sm:table-cell">GB</th><th class="px-4 py-3 text-right hidden md:table-cell">PS/G</th><th class="px-4 py-3 text-right hidden md:table-cell">PA/G</th><th class="px-4 py-3 text-right hidden md:table-cell">DIFF</th></tr></thead><tbody id="standings-body">%s</tbody></table></div>|html}
    rows
...
let games_table (games : game_summary list) =
  let rows =
    ...
    |> List.mapi (fun i (g : game_summary) ->
        ...
        Printf.sprintf
          {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors %s"><td class="px-4 py-3 text-slate-500 font-mono text-sm">%d</td><td class="px-4 py-3 text-slate-400 font-mono text-sm whitespace-nowrap">%s</td><td class="px-4 py-3 font-medium"><div class="flex items-center gap-2 min-w-0">%s<a href="/team/%s" class="player-name min-w-0 hover:text-orange-400 transition-colors">%s</a></div></td><td class="px-4 py-3 text-center font-bold text-orange-400 font-mono whitespace-nowrap">%s - %s<div class="mt-1 text-[10px] font-normal text-slate-500 sm:hidden"><a href="/boxscore/%s" class="hover:text-orange-300">Boxscore</a></div></td><td class="px-4 py-3 text-right font-medium"><div class="flex flex-col items-end gap-1"><div class="flex items-center justify-end gap-2"><a href="/team/%s" class="player-name min-w-0 hover:text-orange-400 transition-colors">%s</a>%s</div></div></td><td class="px-4 py-3 text-right hidden sm:table-cell"><a href="/boxscore/%s" class="text-xs bg-slate-800 hover:bg-slate-700 px-2 py-1 rounded text-slate-400 hover:text-white transition">Boxscore</a></td></tr>|html}
          status_class (i + 1) (escape_html g.game_date) (team_logo_tag ~class_name:"w-4 h-4" g.home_team) (Uri.pct_encode g.home_team) (escape_html g.home_team) score_a score_b (escape_html g.game_id) (Uri.pct_encode g.away_team) (escape_html g.away_team) (team_logo_tag ~class_name:"w-4 h-4" g.away_team) (escape_html g.game_id))
    |> String.concat "\n"
  in
  Printf.sprintf
    {html|<div class="bg-slate-900 rounded-lg border border-slate-800 overflow-x-auto overflow-y-hidden shadow-2xl"><table class="min-w-[680px] sm:min-w-[860px] w-full text-xs sm:text-sm font-mono tabular-nums"><thead class="bg-slate-800/80 text-slate-400 text-[10px] sm:text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-4 py-3 text-left w-10">#</th><th class="px-4 py-3 text-left font-sans w-28 sm:w-32">Date</th><th class="px-4 py-3 text-left font-sans">Home</th><th class="px-4 py-3 text-center w-28 sm:w-32">Score</th><th class="px-4 py-3 text-right font-sans">Away</th><th class="px-4 py-3 text-right font-sans w-24 hidden sm:table-cell">Action</th></tr></thead><tbody id="games-body">%s</tbody></table></div>|html}
    rows
...
let boxscores_table (games : game_summary list) =
  let rows =
    ...
    |> List.mapi (fun i (g : game_summary) ->
        ...
        if g.home_score = None then ""
          ...
          Printf.sprintf
            {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-4 py-3 text-slate-500 font-mono text-sm">%d</td><td class="px-4 py-3 text-slate-400 font-mono text-sm whitespace-nowrap">%s</td><td class="px-4 py-3 font-medium"><div class="flex items-center gap-2 min-w-0">%s<a href="/team/%s" class="player-name min-w-0 hover:text-orange-400 transition-colors">%s</a></div></td><td class="px-4 py-3 text-center font-bold text-white font-mono whitespace-nowrap">%d</td><td class="px-4 py-3 text-center font-bold text-white font-mono whitespace-nowrap">%d</td><td class="px-4 py-3 font-medium text-right"><div class="flex flex-col items-end gap-1"><div class="flex items-center justify-end gap-2"><a href="/team/%s" class="player-name min-w-0 hover:text-orange-400 transition-colors">%s</a>%s</div><a href="/boxscore/%s" class="text-[10px] text-slate-500 hover:text-orange-300 sm:hidden">View</a></div></td><td class="px-4 py-3 text-right font-bold font-mono %s hidden sm:table-cell">%s</td><td class="px-4 py-3 text-right hidden sm:table-cell">
  ...
  in
  Printf.sprintf
    {html|<div class="bg-slate-900 rounded-lg border border-slate-800 overflow-x-auto overflow-y-hidden shadow-2xl"><table class="min-w-[720px] sm:min-w-[900px] w-full text-xs sm:text-sm font-mono tabular-nums"><thead class="bg-slate-800/80 text-slate-400 text-[10px] sm:text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-4 py-3 text-left w-10">#</th><th class="px-4 py-3 text-left w-28 sm:w-32 font-sans">Date</th><th class="px-4 py-3 text-left font-sans">Home</th><th class="px-4 py-3 text-center font-sans">PTS</th><th class="px-4 py-3 text-center font-sans">PTS</th><th class="px-4 py-3 text-right font-sans">Away</th><th class="px-4 py-3 text-right font-sans hidden sm:table-cell">Margin</th><th class="px-4 py-3 text-right w-20 font-sans hidden sm:table-cell">Link</th></tr></thead><tbody id="boxscores-body">%s</tbody></table></div>|html}
    rows
...
let boxscore_player_table (title: string) (players: boxscore_player_stat list) =
  ...
  let rows =
    ...
    |> List.map (fun (p: boxscore_player_stat) ->
        ...
        in
        Printf.sprintf
          {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-3 py-2 font-medium text-white flex items-center gap-3 min-w-0">%s<a href="/player/%s" class="player-name min-w-0 hover:text-orange-400 transition-colors">%s</a>%s</td><td class="px-3 py-2 text-right text-slate-400 font-mono text-sm w-[60px] whitespace-nowrap">%.1f</td><td class="px-3 py-2 text-right text-white font-bold w-[60px] whitespace-nowrap">%d</td><td class="px-3 py-2 text-right font-mono w-[60px] whitespace-nowrap %s">%s</td><td class="px-3 py-2 text-right text-slate-300 w-[60px] whitespace-nowrap">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[60px] whitespace-nowrap">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[60px] whitespace-nowrap hidden sm:table-cell">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[60px] whitespace-nowrap hidden sm:table-cell">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[60px] whitespace-nowrap hidden sm:table-cell">%d</td><td class="px-3 py-2 text-right text-slate-400 text-xs font-mono w-[120px] whitespace-nowrap">%d-%d (%.1f%%)</td><td class="px-3 py-2 text-right text-slate-400 text-xs font-mono w-[120px] whitespace-nowrap">%d-%d (%.1f%%)</td><td class="px-3 py-2 text-right text-slate-400 text-xs font-mono w-[120px] whitespace-nowrap">%d-%d (%.1f%%)</td></tr>|html}
  ...
  in
  Printf.sprintf
    {html|<div class="space-y-3"><h3 class="text-lg font-bold text-white flex items-center gap-2"><span class="w-1 h-6 bg-orange-500 rounded-full"></span>%s</h3><div class="bg-slate-900 rounded-lg border border-slate-800 overflow-x-auto overflow-y-hidden shadow-lg"><table class="w-full min-w-[860px] text-xs sm:text-sm font-mono table-auto"><thead class="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-3 py-2 text-left font-sans">Player</th><th class="px-3 py-2 text-right w-[60px]">MP</th><th class="px-3 py-2 text-right text-orange-400 w-[60px]">PTS</th><th class="px-3 py-2 text-right w-[60px]">+/-</th><th class="px-3 py-2 text-right w-[60px]">TRB</th><th class="px-3 py-2 text-right w-[60px]">AST</th><th class="px-3 py-2 text-right w-[60px] hidden sm:table-cell">STL</th><th class="px-3 py-2 text-right w-[60px] hidden sm:table-cell">BLK</th><th class="px-3 py-2 text-right w-[60px] hidden sm:table-cell">TOV</th><th class="px-3 py-2 text-right w-[120px]">FG</th><th class="px-3 py-2 text-right w-[120px]">3P</th><th class="px-3 py-2 text-right w-[120px]">FT</th></tr></thead><tbody>%s</tbody></table></div></div>|html}
...
let boxscore_page (bs: game_boxscore) =
  ...
  layout ~title:(Printf.sprintf "Boxscore: %s vs %s" gi.gi_home_team_name gi.gi_away_team_name)
    ~content:(Printf.sprintf
      {html|<div class="space-y-8 animate-fade-in"><div class="bg-slate-900 rounded-xl border border-slate-800 p-6 shadow-2xl"><div class="flex flex-col items-center gap-6"><div class="text-slate-500 font-mono text-sm uppercase tracking-widest">%s</div><div class="grid w-full max-w-2xl gap-4 text-center sm:grid-cols-[1fr_auto_1fr] sm:items-center"><div class="flex flex-col items-center gap-2"><div class="flex flex-col items-center gap-2">%s<a href="/team/%s" class="player-name max-w-[160px] sm:max-w-none text-center text-lg sm:text-2xl font-black text-white hover:text-orange-400 transition-colors">%s</a></div><div class="text-slate-400 text-xs tracking-[0.2em]">HOME</div></div><div class="flex flex-col items-center gap-3"><div class="flex items-center gap-6"><div class="text-4xl sm:text-5xl font-black text-white">%d</div><div class="flex flex-col items-center gap-2"><div class="text-xl sm:text-2xl text-slate-700 font-light">vs</div><div class="flex flex-wrap items-center justify-center gap-2">%s%s%s</div></div><div class="text-4xl sm:text-5xl font-black text-white">%d</div></div></div><div class="flex flex-col items-center gap-2"><div class="flex flex-col items-center gap-2">%s<a href="/team/%s" class="player-name max-w-[160px] sm:max-w-none text-center text-lg sm:text-2xl font-black text-white hover:text-orange-400 transition-colors">%s</a></div><div class="text-slate-400 text-xs tracking-[0.2em]">AWAY</div></div></div></div></div>%s<div class="grid grid-cols-1 gap-8">%s%s</div><div class="flex justify-center"><a href="/games" class="text-slate-500 hover:text-orange-500 transition text-sm">← Back to Games</a></div></div>|html}
      (escape_html gi.gi_game_date)
      (team_logo_tag ~class_name:"w-14 h-14 sm:w-16 sm:h-16" gi.gi_home_team_name)
      (Uri.pct_encode gi.gi_home_team_name)
      (escape_html gi.gi_home_team_name)
      gi.gi_home_score
      margin_badge
      quality_badge
      pbp_link
      gi.gi_away_score
      (team_logo_tag ~class_name:"w-14 h-14 sm:w-16 sm:h-16" gi.gi_away_team_name)
      (Uri.pct_encode gi.gi_away_team_name)
      (escape_html gi.gi_away_team_name)
      data_notes
...
let transactions_page
  ...
  <td class="px-4 py-3 min-w-0"><a class="text-slate-200 hover:text-orange-400 font-bold truncate block" href="/player/%s">%s</a><div class="mt-1 text-[11px] text-slate-500 font-mono">%s</div><a class="mt-1 text-[10px] text-slate-500 underline font-mono sm:hidden" href="%s" target="_blank" rel="noreferrer">Source</a></td>
  <td class="px-4 py-3">%s</td>
  <td class="px-4 py-3 text-[11px] text-slate-300 font-mono whitespace-pre-line break-words hidden sm:table-cell">%s</td>
  <td class="px-4 py-3 text-[11px] hidden sm:table-cell"><a class="text-slate-500 hover:text-slate-300 underline font-mono" href="%s" target="_blank" rel="noreferrer">Source</a></td>
</tr>|html}
                (escape_html y)
  ...
  <table class="min-w-[520px] sm:min-w-[980px] w-full text-sm table-fixed tabular-nums">
    <thead class="bg-slate-800/80 text-slate-400 uppercase tracking-wider text-[10px] whitespace-nowrap">
      <tr>
        ...
        <th class="px-4 py-3 text-left hidden sm:table-cell">Raw</th>
        <th class="px-4 py-3 text-left w-[90px] hidden sm:table-cell">Link</th>
      </tr>
    </thead>
    <tbody>%s</tbody>
  </table>
