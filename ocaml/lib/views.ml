...
let player_row ?(show_player_id=false) ?(team_cell_class="px-3 py-2") (rank: int) (p: player_aggregate) =
  let id_badge =
  ...
  let team_cell = Printf.sprintf {html|<td class="%s">%s</td>|html} (escape_html team_cell_class) (team_badge p.team_name) in
  Printf.sprintf
...
let player_profile_page (profile: player_profile) ~scope ~(seasons_catalog: season_info list) =
  ...
  let data_notes_html =
    ...
    let pbp_card_html =
      Printf.sprintf
        {html|<div class="bg-slate-800/40 border border-slate-700/50 rounded-lg p-5 min-w-0"><div class="flex flex-wrap items-center justify-between gap-3"><div class="text-slate-400 font-bold uppercase tracking-widest text-[11px] flex items-center gap-2"><span class="text-base">🎥</span> PBP +/-</div><span class="px-2 py-0.5 rounded bg-slate-800/60 border border-slate-700/60 text-[10px] font-mono text-slate-300 whitespace-nowrap">부분</span></div>%s<div class="mt-3 text-slate-400 text-xs leading-relaxed"><div><span class="font-mono text-slate-300">+/-</span>: 문자중계(PBP) 기반</div><div><span class="font-mono text-slate-300">M</span>: PBP가 없을 때 팀 득실마진(경기 최종 점수)</div><div><span class="font-mono text-slate-300">-</span>: 데이터 없음/품질 이슈</div></div></div>|html}
        pbp_stat_line
    in
    let draft_card_html =
      {html|<div class="bg-slate-800/40 border border-slate-700/50 rounded-lg p-5 min-w-0"><div class="flex items-center justify-between gap-3"><div class="text-slate-400 font-bold uppercase tracking-widest text-[11px] flex items-center gap-2"><span class="text-base">🧩</span> Draft / Trade</div><span class="px-2 py-0.5 rounded bg-slate-800/60 border border-slate-700/60 text-[10px] font-mono text-slate-300">예정</span></div><div class="mt-3 text-slate-400 text-xs leading-relaxed">공식 드래프트/이적 기록은 아직 수집 중입니다. 현재는 <span class="font-mono text-slate-300">박스스코어 출전팀</span> 변화로 팀 이동을 추정합니다. (공식 페이지 기반 추가 수집 필요)</div></div>|html}
    in
    Printf.sprintf
      {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-6 shadow-lg"><div class="flex items-start justify-between gap-4 mb-4"><h3 class="text-slate-300 font-bold uppercase tracking-wider text-xs flex items-center gap-2"><span class="text-lg">🧾</span> Data Notes</h3><span class="text-[11px] text-slate-500 font-mono">커버리지</span></div><div class="grid grid-cols-1 md:grid-cols-2 gap-4">%s%s</div><div class="mt-4">%s</div></div>|html}
      seasons_card_html
      pbp_card_html
      draft_card_html
  in
