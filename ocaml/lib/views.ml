...
let player_img_tag ?(class_name="w-12 h-12") player_id _player_name =
  ...
  Printf.sprintf
    {html|<img src="%s" alt="" aria-hidden="true" class="%s rounded-full object-cover bg-slate-800 border border-slate-700 shadow-sm" loading="lazy" data-placeholder="%s" onerror="if(!this.dataset.placeholderApplied){this.dataset.placeholderApplied='1';this.src=this.dataset.placeholder;}">|html}
    (escape_html src)
...
let player_season_stats_table ~scope (stats: season_stats list) =
  ...
  let row_html ?(highlight=false) (s: season_stats) =
    ...
    let name_class = if highlight then "text-white font-black" else "text-white font-medium" in
        Printf.sprintf {html|<tr class="%s">
          ...
          <td class="px-4 py-3 text-right text-slate-300 w-[60px] font-mono whitespace-nowrap">%d</td>
          <td class="px-4 py-3 text-right text-slate-300 w-[80px] font-mono whitespace-nowrap hidden sm:table-cell">%.1f</td>
          <td class="px-4 py-3 text-right font-bold text-orange-400 w-[80px] font-mono whitespace-nowrap">%.1f</td>
          <td class="px-4 py-3 text-right font-bold %s w-[80px] font-mono whitespace-nowrap hidden sm:table-cell">%s</td>
          <td class="px-4 py-3 text-right text-slate-300 w-[80px] font-mono whitespace-nowrap">%.1f</td>
          <td class="px-4 py-3 text-right text-slate-300 w-[80px] font-mono whitespace-nowrap hidden sm:table-cell">%.1f</td>
          <td class="px-4 py-3 text-right text-slate-300 w-[80px] font-mono whitespace-nowrap">%.1f</td>
        </tr>|html}
      row_class
  ...
  let rows =
    ...
    |> String.concat "\n"
  in
  Printf.sprintf {html|<div class="bg-slate-900 rounded-xl border border-slate-800 overflow-x-auto shadow-lg animate-fade-in"><table class="season-stats-table min-w-[520px] sm:min-w-[720px] w-full text-sm font-mono table-auto">
    <thead class="bg-slate-800/80 text-slate-400 uppercase tracking-wider text-xs whitespace-nowrap">
      <tr>
...
let player_season_stats_component ~player_id ~scope (stats: season_stats list) =
  let btn_class active =
    if active then "season-stats-tab px-3 sm:px-4 py-2 bg-transparent border-0 border-b-2 border-orange-500 text-white font-medium cursor-default pointer-events-none whitespace-nowrap"
    else "season-stats-tab px-3 sm:px-4 py-2 bg-transparent border-0 border-b-2 border-transparent text-slate-400 hover:text-white transition cursor-pointer font-medium whitespace-nowrap"
  in
  ...
  let tabs = Printf.sprintf
    {html|<div id="season-stats-tabs" class="flex items-center gap-2 sm:gap-4 border-b border-slate-800 mb-4 overflow-x-auto no-scrollbar whitespace-nowrap -mx-4 px-4 sm:mx-0 sm:px-0">
...
let career_highs_card (ch: career_high_item list option) =
  ...
  | Some items ->
      let items_html =
        ...
        |> String.concat "\n"
      in
      Printf.sprintf {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-4 sm:p-6 shadow-lg h-full"><h3 class="text-orange-400 font-bold uppercase tracking-wider text-xs mb-4 flex items-center gap-2"><span class="text-lg">🚀</span> Career Highs</h3><div class="space-y-3">%s</div></div>|html} items_html
...
let player_row ?(show_player_id=false) ?(team_cell_class="px-3 py-2 w-[120px] sm:w-[160px]") ?(include_team=true) (rank: int) (p: player_aggregate) =
  ...
  Printf.sprintf
    {html|<tr class="group border-b border-slate-700/50 hover:bg-slate-800/50 transition-colors">
      ...
      <td class="px-3 py-2 font-medium text-white w-[220px] sm:w-[260px]">
      ...
      %s
      <td class="px-3 py-2 text-right text-slate-400 whitespace-nowrap hidden sm:table-cell">%d</td>
      %s%s%s%s%s%s%s%s
    </tr>|html}
    rank
...
let players_table (players: player_aggregate list) =
  ...
  Printf.sprintf
    ...
    <table class="min-w-[680px] sm:min-w-[860px] lg:min-w-[980px] w-full text-xs sm:text-sm font-mono tabular-nums table-auto">
      <thead class="bg-slate-800/80 sticky top-0 text-slate-400 text-[10px] sm:text-xs uppercase tracking-wider whitespace-nowrap">
        <tr>
          ...
          <th class="px-3 py-3 text-right w-[72px] cursor-pointer hover:text-orange-400" hx-get="/players/table?sort=pts" hx-target="#players-table" hx-swap="innerHTML" hx-include="#players-filter">PTS</th>
          <th class="px-3 py-3 text-right w-[72px] cursor-pointer hover:text-orange-400 hidden md:table-cell" title="MG: 팀 득실마진(출전시간 가중 평균)" hx-get="/players/table?sort=mg" hx-target="#players-table" hx-swap="innerHTML" hx-include="#players-filter">MG</th>
          <th class="px-3 py-3 text-right w-[72px] cursor-pointer hover:text-orange-400" hx-get="/players/table?sort=reb" hx-target="#players-table" hx-swap="innerHTML" hx-include="#players-filter">REB</th>
          <th class="px-3 py-3 text-right w-[72px] cursor-pointer hover:text-orange-400 hidden md:table-cell" hx-get="/players/table?sort=ast" hx-target="#players-table" hx-swap="innerHTML" hx-include="#players-filter">AST</th>
          ...
          <th class="px-3 py-3 text-right w-[72px] cursor-pointer hover:text-orange-400" hx-get="/players/table?sort=eff" hx-target="#players-table" hx-swap="innerHTML" hx-include="#players-filter">EFF</th>
        </tr>
      </thead>
...
<body class="bg-[#0b0e14] text-slate-200 font-sans antialiased min-h-screen is-loading">
  ...
  <header class="sticky top-0 z-50 bg-slate-900/95 backdrop-blur border-b border-slate-800 px-4 sm:px-6 py-3 sm:py-4">
    <div class="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <nav class="flex items-center gap-3 sm:gap-x-4 sm:gap-y-2 text-xs sm:text-sm overflow-x-auto no-scrollbar whitespace-nowrap -mx-4 px-4 sm:mx-0 sm:px-0 flex-nowrap sm:flex-wrap sm:justify-end sm:overflow-visible">
        <a href="/" class="shrink-0 text-slate-400 hover:text-white transition">Home</a>
        <a href="/awards" class="shrink-0 text-slate-400 hover:text-white transition">Awards</a>
        <a href="/leaders" class="shrink-0 text-slate-400 hover:text-white transition">Leaders</a>
        <a href="/boxscores" class="shrink-0 text-slate-400 hover:text-white transition">Boxscores</a>
        <a href="/games" class="shrink-0 text-slate-400 hover:text-white transition">Games</a>
        <a href="/standings" class="shrink-0 text-slate-400 hover:text-white transition">Standings</a>
        <a href="/teams" class="shrink-0 text-slate-400 hover:text-white transition">Teams</a>
...
let teams_table ~season ~scope (stats: team_stats list) =
  ...
  Printf.sprintf
    {html|<div class="bg-slate-900 rounded-lg border border-slate-800 overflow-x-auto overflow-y-hidden"><table class="w-full min-w-max text-xs sm:text-sm font-mono tabular-nums"><thead class="bg-slate-800/80 text-slate-400 text-[10px] sm:text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-3 py-2 text-left">Team</th><th class="px-3 py-2 text-right">GP</th><th class="px-3 py-2 text-right hidden md:table-cell">%s</th><th class="px-3 py-2 text-right">PTS</th><th class="px-3 py-2 text-right" title="MG: 팀 득실마진(PTS - PA)">MG</th><th class="px-3 py-2 text-right hidden md:table-cell">PA</th><th class="px-3 py-2 text-right hidden sm:table-cell">REB</th><th class="px-3 py-2 text-right hidden sm:table-cell">AST</th><th class="px-3 py-2 text-right hidden md:table-cell">STL</th><th class="px-3 py-2 text-right hidden md:table-cell">BLK</th><th class="px-3 py-2 text-right hidden md:table-cell">TO</th><th class="px-3 py-2 text-right hidden lg:table-cell">FG%%</th><th class="px-3 py-2 text-right hidden lg:table-cell">3P%%</th><th class="px-3 py-2 text-right hidden lg:table-cell">FT%%</th><th class="px-3 py-2 text-right hidden lg:table-cell">eFG%%</th><th class="px-3 py-2 text-right text-orange-400">EFF</th></tr></thead><tbody>%s</tbody></table></div>|html}
    min_label rows
...
let standings_table ~season (standings : team_standing list) =
  let rows =
    ...
    |> List.mapi (fun i (s : team_standing) ->
        ...
        in
        Printf.sprintf
          {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-4 py-3 text-slate-500 font-mono text-sm">%d</td><td class="px-4 py-3 font-bold text-white flex items-center gap-2">%s<a href="%s" class="hover:text-orange-400 transition-colors">%s</a></td><td class="px-4 py-3 text-right text-slate-300">%d</td><td class="px-4 py-3 text-right text-slate-300">%d</td><td class="px-4 py-3 text-right text-slate-300">%d</td><td class="px-4 py-3 text-right text-orange-400 font-bold">%s</td><td class="px-4 py-3 text-right text-slate-400 hidden sm:table-cell">%s</td><td class="px-4 py-3 text-right text-slate-300 font-mono hidden md:table-cell">%.1f</td><td class="px-4 py-3 text-right text-slate-300 font-mono hidden md:table-cell">%.1f</td><td class="px-4 py-3 text-right %s font-mono font-bold hidden sm:table-cell">%.1f</td></tr>|html}
...
let games_table (games : game_summary list) =
    ...
    |> List.mapi (fun i (g : game_summary) ->
        ...
        let action_html =
          if g.home_score = None then
            ...
            Printf.sprintf
              {html|<a href="/boxscore/%s" class="text-[10px] bg-slate-800 hover:bg-slate-700 px-2 py-1 rounded text-slate-300 hover:text-white transition">Boxscore</a>|html}
              (escape_html g.game_id)
        in
        Printf.sprintf
          {html|<div class="bg-slate-900/60 border border-slate-800 rounded-lg p-3 shadow-sm space-y-2">
  ...
  <div class="flex items-center justify-between gap-3">
    <div class="flex flex-col gap-1 min-w-0">
      <div class="flex items-center gap-2 text-sm font-medium">%s<a href="/team/%s" class="hover:text-orange-400 transition-colors truncate">%s</a></div>
      <div class="flex items-center gap-2 text-sm font-medium">%s<a href="/team/%s" class="hover:text-orange-400 transition-colors truncate">%s</a></div>
    </div>
  ...
  let rows =
    ...
    |> List.mapi (fun i (g : game_summary) ->
        ...
        let status_class = if g.home_score = None then "text-slate-500" else "text-white" in
        Printf.sprintf
          {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors %s"><td class="px-4 py-3 text-slate-500 font-mono text-sm hidden sm:table-cell">%d</td><td class="px-4 py-3 text-slate-400 font-mono text-sm w-[90px] sm:w-32 whitespace-nowrap">%s</td><td class="px-4 py-3 font-medium flex items-center gap-2">%s<a href="/team/%s" class="hover:text-orange-400 transition-colors">%s</a></td><td class="px-4 py-3 text-center font-bold text-orange-400 font-mono">%s - %s</td><td class="px-4 py-3 text-right font-medium"><div class="flex items-center justify-end gap-2"><a href="/team/%s" class="hover:text-orange-400 transition-colors">%s</a>%s</div></td><td class="px-4 py-3 text-right w-[64px] sm:w-24"><a href="/boxscore/%s" class="text-[10px] sm:text-xs bg-slate-800 hover:bg-slate-700 px-2 py-1 rounded text-slate-400 hover:text-white transition">Boxscore</a></td></tr>|html}
...
let boxscores_table (games : game_summary list) =
  let mobile_cards =
    ...
    |> List.mapi (fun i (g : game_summary) ->
        ...
        let margin_color = if margin > 0 then "text-sky-400" else if margin < 0 then "text-rose-400" else "text-slate-400" in
        if g.home_score = None then ""
        else
          Printf.sprintf
            {html|<div class="bg-slate-900/60 border border-slate-800 rounded-lg p-3 shadow-sm space-y-2">
  <div class="flex items-center justify-between text-[11px] text-slate-500 font-mono">
    <span>#%d · %s</span>
    <span class="px-2 py-0.5 rounded bg-slate-800/60 border border-slate-700/60 text-[10px] font-mono %s whitespace-nowrap">Δ %s</span>
  </div>
  <div class="flex items-center justify-between gap-3">
    <div class="flex flex-col gap-1 min-w-0 w-full">
      <div class="flex items-center gap-2 text-sm font-medium w-full">%s<a href="/team/%s" class="hover:text-orange-400 transition-colors truncate">%s</a><span class="ml-auto font-mono text-slate-200">%d</span></div>
      <div class="flex items-center gap-2 text-sm font-medium w-full">%s<a href="/team/%s" class="hover:text-orange-400 transition-colors truncate">%s</a><span class="ml-auto font-mono text-slate-200">%d</span></div>
    </div>
    <a href="/boxscore/%s" class="text-[10px] bg-slate-800 hover:bg-slate-700 px-2 py-1 rounded text-slate-300 hover:text-white transition">View</a>
  </div>
</div>|html}
            (i + 1)
            (escape_html g.game_date)
  ...
  let rows =
    ...
    |> List.mapi (fun i (g : game_summary) ->
        ...
        if g.home_score = None then ""
          ...
          Printf.sprintf
            {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-4 py-3 text-slate-500 font-mono text-sm hidden sm:table-cell">%d</td><td class="px-4 py-3 text-slate-400 font-mono text-sm w-[90px] sm:w-32 whitespace-nowrap">%s</td><td class="px-4 py-3 font-medium flex items-center gap-2">%s<a href="/team/%s" class="hover:text-orange-400 transition-colors">%s</a></td><td class="px-4 py-3 text-center font-bold text-white font-mono">%d</td><td class="px-4 py-3 text-center font-bold text-white font-mono">%d</td><td class="px-4 py-3 font-medium text-right"><div class="flex items-center justify-end gap-2"><a href="/team/%s" class="hover:text-orange-400 transition-colors">%s</a>%s</div></td><td class="px-4 py-3 text-right font-bold font-mono %s hidden sm:table-cell">%s</td><td class="px-4 py-3 text-right w-[60px] sm:w-20">
                  <a href="/boxscore/%s" class="text-[10px] sm:text-xs bg-slate-800 hover:bg-slate-700 px-2 py-1 rounded text-slate-400 hover:text-white transition">View</a>
                </td></tr>|html}
...
let boxscore_player_table (title: string) (players: boxscore_player_stat list) =
  ...
  let rows =
    ...
    |> List.map (fun (p: boxscore_player_stat) ->
        ...
        in
        Printf.sprintf
          {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-3 py-2 font-medium text-white flex items-center gap-3 min-w-[160px]">%s<a href="/player/%s" class="hover:text-orange-400 transition-colors">%s</a>%s</td><td class="px-3 py-2 text-right text-slate-400 font-mono text-sm w-[56px] sm:w-[60px] hidden sm:table-cell">%.1f</td><td class="px-3 py-2 text-right text-white font-bold w-[56px] sm:w-[60px]">%d</td><td class="px-3 py-2 text-right font-mono w-[56px] sm:w-[60px] %s hidden sm:table-cell">%s</td><td class="px-3 py-2 text-right text-slate-300 w-[56px] sm:w-[60px]">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[56px] sm:w-[60px]">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[56px] sm:w-[60px] hidden md:table-cell">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[56px] sm:w-[60px] hidden md:table-cell">%d</td><td class="px-3 py-2 text-right text-slate-300 w-[56px] sm:w-[60px] hidden md:table-cell">%d</td><td class="px-3 py-2 text-right text-slate-400 text-xs font-mono w-[120px] hidden lg:table-cell">%d-%d (%.1f%%)</td><td class="px-3 py-2 text-right text-slate-400 text-xs font-mono w-[120px] hidden lg:table-cell">%d-%d (%.1f%%)</td><td class="px-3 py-2 text-right text-slate-400 text-xs font-mono w-[120px] hidden lg:table-cell">%d-%d (%.1f%%)</td></tr>|html}
...
let boxscore_page (bs: game_boxscore) =
  ...
  let pbp_link =
    Printf.sprintf
      {html|<a href="/boxscore/%s/pbp" class="px-2 py-1 rounded-full bg-slate-800/70 border border-slate-700 text-[10px] font-mono tracking-wider text-slate-300 hover:text-white hover:border-slate-500 transition">PBP</a>|html}
  ...
  layout ~title:(Printf.sprintf "Boxscore: %s vs %s" gi.gi_home_team_name gi.gi_away_team_name)
    ~content:(Printf.sprintf
      {html|<div class="space-y-8 animate-fade-in"><div class="bg-slate-900 rounded-xl border border-slate-800 p-6 shadow-2xl"><div class="flex flex-col items-center gap-6"><div class="text-slate-500 font-mono text-sm uppercase tracking-widest">%s</div><div class="flex items-center justify-between w-full max-w-2xl gap-3 sm:gap-6"><div class="flex flex-col items-center gap-2 sm:gap-3 min-w-0"><div class="text-lg sm:text-2xl font-black text-white flex items-center gap-2 sm:gap-3 min-w-0">%s<a href="/team/%s" class="hover:text-orange-400 transition-colors truncate max-w-[120px] sm:max-w-none whitespace-nowrap">%s</a></div><div class="text-[11px] sm:text-sm text-slate-400">HOME</div></div><div class="flex items-center gap-4 sm:gap-8"><div class="text-4xl sm:text-5xl font-black text-white">%d</div><div class="flex flex-col items-center gap-1.5 sm:gap-2"><div class="text-lg sm:text-2xl text-slate-700 font-light">vs</div><div class="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2">%s%s%s</div></div><div class="text-4xl sm:text-5xl font-black text-white">%d</div></div><div class="flex flex-col items-center gap-2 sm:gap-3 min-w-0"><div class="text-lg sm:text-2xl font-black text-white flex items-center gap-2 sm:gap-3 min-w-0"><a href="/team/%s" class="hover:text-orange-400 transition-colors truncate max-w-[120px] sm:max-w-none whitespace-nowrap">%s</a>%s</div><div class="text-[11px] sm:text-sm text-slate-400">AWAY</div></div></div></div></div>%s<div class="grid grid-cols-1 gap-8">%s%s</div><div class="flex justify-center"><a href="/games" class="text-slate-500 hover:text-orange-500 transition text-sm">← Back to Games</a></div></div>|html}
...
let pbp_page ~(game: game_info) ~(periods: string list) ~(selected_period: string) ~(events: pbp_event list) =
  ...
  let tabs =
    ...
    | _ ->
        ...
        |> List.map (fun p ->
            ...
            let cls =
              if is_selected then "bg-orange-500/15 border-orange-500/30 text-orange-300"
              else "bg-slate-800/40 border-slate-700/40 text-slate-400 hover:text-slate-200"
            in
            Printf.sprintf
  ...
  let rows =
    ...
    |> List.map (fun (e: pbp_event) ->
        ...
        let side_badge =
          ...
          | _ ->
              {html|<div class="flex items-center gap-2"><div class="w-6 h-6 rounded bg-slate-800 flex items-center justify-center text-xs">🏀</div><span class="text-[10px] font-mono text-slate-500 tracking-wider">—</span></div>|html}
        in
        let score_badge =
          match e.pe_team1_score, e.pe_team2_score with
          | Some a, Some h ->
              Printf.sprintf
                {html|<span class="px-2 py-0.5 rounded bg-slate-800/40 border border-slate-700/40 text-[10px] font-mono text-slate-300 whitespace-nowrap" title="AWAY %s / HOME %s">A %d - %d H</span>|html}
        ...
        let row_cls =
          if is_score then "bg-slate-800/15"
          else "hover:bg-slate-800/30"
        in
        Printf.sprintf
          {html|<li class="flex items-start gap-3 px-4 py-3 border-b border-slate-800/60 last:border-0 %s transition-colors">
            <div class="w-[56px] text-right text-xs font-mono text-slate-500 pt-1">%s</div>
            <div class="min-w-0 flex-1">
              <div class="flex flex-wrap items-center gap-2">
                %s
                %s
                <span class="text-sm text-slate-200 break-words">%s</span>
              </div>
            </div>
          </li>|html}
          row_cls
  ...
  let body =
    ...
    | [] ->
        Printf.sprintf
          {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-6 text-slate-400 text-sm">
            <div class="font-bold text-white mb-2">PBP 데이터가 없습니다</div>
            <div class="leading-relaxed">
              WKBL 문자중계(PBP)가 제공되지 않았거나 아직 수집되지 않은 경기입니다.
              개인 <span class="font-mono text-slate-200">+/-</span> 계산도 PBP 기반이라 이 경기에는 표시되지 않을 수 있습니다.
            </div>
          </div>|html}
    | _ ->
        Printf.sprintf
          {html|<div class="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-xl">%s<ul class="divide-y divide-slate-800/60">%s</ul></div>|html}
          tabs
          rows
  in
  layout
      ...
      {html|<div class="space-y-6 animate-fade-in">
        <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div class="space-y-2">
            ...
            <h2 class="text-2xl font-black text-white">Play-by-Play</h2>
          ...
          <div class="flex items-center gap-3">
            %s
            <a href="/boxscore/%s" class="px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700 text-sm text-slate-200 hover:text-white hover:border-slate-500 transition">← Boxscore</a>
          </div>
        </div>
        <div class="text-xs text-slate-500 leading-relaxed">
          <span class="font-mono text-slate-200">team1/team2</span> 스코어는 WKBL 문자중계(XML) 기준이며, 일반적으로 <span class="font-mono text-slate-200">A(원정)</span> / <span class="font-mono text-slate-200">H(홈)</span> 순으로 표시됩니다.
        </div>
        %s
      </div>|html}
...
let leader_card_signed title (leaders: leader_entry list) =
  if leaders = [] then ""
    ...
    let others_rows =
      ...
      |> List.mapi (fun i l ->
          let id_badge = if show_id l then Printf.sprintf {html|<span class="ml-2">%s</span>|html} (player_id_badge l.le_player_id) else "" in
          Printf.sprintf {html|<div class="flex items-center justify-between py-2 border-b border-slate-800/60 last:border-0"><div class="flex items-center gap-3"><span class="text-slate-500 font-mono text-sm w-4">%d</span>%s<div class="flex flex-col"><div class="text-sm font-medium text-slate-300"><a href="/player/%s" class="hover:text-orange-400 transition-colors">%s</a>%s</div><div class="text-xs text-slate-500">%s</div></div></div><div class="font-mono font-bold text-slate-400">%s</div></div>|html} (i + 2) (player_img_tag ~class_name:"w-8 h-8" l.le_player_id l.le_player_name) l.le_player_id (escape_html (normalize_name l.le_player_name)) id_badge (escape_html l.le_team_name) (escape_html (signed l.le_stat_value)))
      |> String.concat "\n"
    in
    Printf.sprintf {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-5 shadow-lg"><h3 class="text-slate-400 font-bold uppercase tracking-wider text-xs mb-4">%s</h3><div class="flex items-center gap-4 mb-6 pb-6 border-b border-slate-800"><div class="relative">%s<div class="absolute -top-2 -right-2 bg-orange-500 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center border-2 border-slate-900">1</div></div><div><div class="text-3xl font-black text-white">%s</div><div class="font-bold text-orange-400 flex items-center flex-wrap"><a href="/player/%s" class="hover:text-white transition-colors">%s</a>%s</div><div class="text-xs text-slate-500">%s</div></div></div><div class="space-y-1">%s</div></div>|html}
      (escape_html title)
...
let awards_page ~(season: string) ~(seasons: season_info list) ~(include_mismatch: bool) ~(prev_season_name: string option) ~(mvp: leader_entry list) ~(mip: leader_entry list) =
  ...
  let mvp_card =
    if mvp = [] then
      {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-5 shadow-lg"><h3 class="text-slate-400 font-bold uppercase tracking-wider text-xs mb-4">MVP (EFF)</h3><div class="text-slate-400 text-sm leading-relaxed">표시할 데이터가 없습니다.</div></div>|html}
    else
  ...
  let mip_card =
    ...
    | None ->
        Printf.sprintf
          {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-5 shadow-lg"><h3 class="text-slate-400 font-bold uppercase tracking-wider text-xs mb-4">MIP (ΔEFF)</h3><div class="text-slate-400 text-sm leading-relaxed">전 시즌 대비 <span class="font-mono text-slate-300">EFF(AVG(game_score))</span> 변화(Δ)를 계산합니다. 시즌 선택 시에만 표시됩니다.</div></div>|html}
    | Some prev_name ->
        if mip = [] then
          Printf.sprintf
            {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-5 shadow-lg"><h3 class="text-slate-400 font-bold uppercase tracking-wider text-xs mb-4">MIP (ΔEFF)</h3><div class="text-slate-400 text-sm leading-relaxed">%s 대비 ΔEFF 계산에 필요한 표본이 부족합니다. (두 시즌 모두 <span class="font-mono text-slate-300">GP≥10</span>)</div></div>|html}
            (escape_html prev_name)
        else
          leader_card_signed (Printf.sprintf "MIP (ΔEFF vs %s)" prev_name) mip
  in
  let disclaimer_html =
    {html|<div class="bg-slate-900/50 rounded-xl border border-slate-800/60 p-5 text-slate-400 text-sm leading-relaxed"><div class="font-bold text-slate-300 mb-2">Stat Awards (unofficial)</div><ul class="list-disc list-inside space-y-1"><li>MVP: <span class="font-mono text-slate-300">EFF = AVG(game_score)</span> 상위</li><li>MIP: 전 시즌 대비 <span class="font-mono text-slate-300">ΔEFF</span> 상위 (두 시즌 모두 <span class="font-mono text-slate-300">GP≥10</span>)</li><li>공식 수상 데이터가 DB에 없어서, 현재는 박스스코어 기반 지표로만 추정합니다.</li></ul></div>|html}
  in
  layout
    ~title:"WKBL Awards"
    ~content:(Printf.sprintf
      {html|<div class="space-y-8 animate-fade-in"><div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><h2 class="text-3xl font-black text-white">Awards</h2><p class="text-slate-400">Stat-based awards for quick reference.</p></div><form action="/awards" method="get" class="flex flex-wrap items-center gap-3"><select name="season" class="bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm focus:border-orange-500 focus:outline-none w-40" onchange="this.form.submit()">%s</select><label class="flex items-center gap-2 text-xs text-slate-400"><input type="checkbox" name="include_mismatch" value="1" %s class="h-4 w-4 rounded border-slate-600 bg-slate-800 accent-orange-500" onchange="this.form.submit()" title="Final score != sum(points) 경기 포함"><span>Mismatch 포함</span></label></form></div><div class="grid grid-cols-1 md:grid-cols-2 gap-6">%s%s</div>%s</div>|html}
      season_options
      include_checked
      mvp_card
      mip_card
...
let player_profile_page ?(leaderboards=None) (profile: player_profile) ~scope ~(seasons_catalog: season_info list) =
  ...
  let game_rows games =
    ...
    |> List.map (fun (g: player_game_stat) ->
        ...
        Printf.sprintf
          {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-4 py-3 text-slate-400 text-sm font-mono whitespace-nowrap w-[90px] sm:w-[110px]"><a href="/boxscore/%s" class="hover:text-orange-400 transition-colors">%s</a></td><td class="px-4 py-3 text-white"><div class="flex flex-wrap items-center gap-x-3 gap-y-2"><a href="%s" class="player-name min-w-0 flex-1 hover:text-orange-400 transition-colors">%s</a><div class="flex items-center gap-2 shrink-0">%s%s</div></div></td><td class="px-4 py-3 text-right font-mono text-slate-400 w-[60px] sm:w-[72px] whitespace-nowrap">%.1f</td><td class="px-4 py-3 text-right font-bold %s w-[60px] sm:w-[72px] whitespace-nowrap">%d</td><td class="px-4 py-3 text-right font-mono w-[60px] sm:w-[72px] whitespace-nowrap %s">%s</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px] whitespace-nowrap">%d</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px] whitespace-nowrap hidden sm:table-cell">%d</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px] whitespace-nowrap hidden sm:table-cell">%d</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px] whitespace-nowrap hidden sm:table-cell">%d</td></tr>|html}
  ...
  let all_star_section_html =
    if profile.all_star_games = [] then
      ...
      Printf.sprintf
        {html|<div class="space-y-4"><div class="flex flex-col gap-1"><h3 class="text-xl font-bold text-white">All-Star Games</h3><p class="text-[11px] text-slate-500">올스타전(<span class="font-mono text-slate-300">game_type=10</span>) 기록은 시즌/커리어 테이블에서 제외하고 여기서만 보여줍니다.</p></div><div class="bg-slate-900 rounded-xl border border-slate-800 overflow-x-auto shadow-lg"><table class="min-w-[680px] sm:min-w-[920px] w-full text-sm font-mono table-auto"><thead class="bg-slate-800/80 text-slate-400 uppercase tracking-wider text-xs whitespace-nowrap"><tr><th class="px-4 py-3 text-left font-sans w-[90px] sm:w-[110px]">Date</th><th class="px-4 py-3 text-left font-sans">Opponent</th><th class="px-4 py-3 text-right w-[60px] sm:w-[72px]">MIN</th><th class="px-4 py-3 text-right text-orange-400 w-[60px] sm:w-[72px]">PTS</th><th class="px-4 py-3 text-right w-[60px] sm:w-[72px]">+/-</th><th class="px-4 py-3 text-right w-[60px] sm:w-[72px]">REB</th><th class="px-4 py-3 text-right w-[60px] sm:w-[72px] hidden sm:table-cell">AST</th><th class="px-4 py-3 text-right w-[60px] sm:w-[72px] hidden sm:table-cell">STL</th><th class="px-4 py-3 text-right w-[60px] sm:w-[72px] hidden sm:table-cell">BLK</th></tr></thead><tbody>%s</tbody></table></div></div>|html}
        all_star_rows
  in
    ...
    | stints ->
        ...
        let total_stints = List.length stints_asc in
	        let stint_rows =
              ...
              let current_chip =
                if is_current then
                  {html|<span class="px-2 py-0.5 rounded bg-orange-500/10 border border-orange-500/30 text-[10px] font-mono text-orange-300 whitespace-nowrap">Current</span>|html}
                else
                  ""
              in
              Printf.sprintf
	        ...
	        in
	        Printf.sprintf
	          {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-4 sm:p-6 shadow-lg"><div class="flex items-start justify-between gap-4 mb-4"><div class="min-w-0"><h3 class="text-slate-300 font-bold uppercase tracking-wider text-xs flex items-center gap-2"><span class="text-lg">🔁</span> Team Movement</h3><div class="mt-1 text-[11px] text-slate-500 leading-relaxed break-words">박스스코어 출전팀 변화로 추정한 연보입니다. (기간=첫/마지막 출전일)</div></div><span class="text-[11px] text-slate-500 font-mono shrink-0">박스스코어</span></div><div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3 gap-3 text-xs"><div class="bg-slate-800/40 border border-slate-700/50 rounded-lg p-3 min-w-0"><div class="text-slate-500 font-mono uppercase tracking-widest text-[11px]">Current</div><div class="mt-2 text-slate-300 min-w-0">%s</div></div><div class="bg-slate-800/40 border border-slate-700/50 rounded-lg p-3"><div class="text-slate-500 font-mono uppercase tracking-widest text-[11px]">Transfers</div><div class="mt-2 font-mono text-slate-200 text-lg font-black">%d</div></div><div class="bg-slate-800/40 border border-slate-700/50 rounded-lg p-3 min-w-0"><div class="text-slate-500 font-mono uppercase tracking-widest text-[11px]">Latest</div><div class="mt-2 text-slate-300 min-w-0">%s</div></div></div><div class="mt-4 rounded-lg border border-slate-800/60 bg-slate-950/30 p-4"><ol class="relative border-l border-slate-800/60 ml-2 space-y-4">%s</ol></div>%s</div>|html}
  ...
  let leaderboards_html =
    ...
    | Some (lb_season_code, lb_season_name, lb_leaders) ->
	  ...
	  let data_notes_html =
        ...
        Printf.sprintf
          {html|<span class="inline-flex flex-wrap items-baseline gap-x-1 gap-y-1"><span class="text-slate-300 whitespace-nowrap">%s</span><span class="whitespace-nowrap"><span class="text-slate-600">→</span><span class="ml-1 text-slate-300 whitespace-nowrap">%s</span></span></span>|html}
          (escape_html oldest)
          (escape_html latest)
    in
    ...
    let backfill_row_html =
      if show_backfill then
        Printf.sprintf
          {html|<div class="text-slate-500 leading-relaxed">현재 DB에 %d시즌 정도만 있어요. 더 채우려면 아래를 실행하세요:</div><code class="mt-2 block font-mono text-slate-300 bg-slate-800/40 border border-slate-700/60 px-3 py-2 rounded overflow-x-auto whitespace-nowrap">python3 scripts/wkbl_refresh_all.py -- --years 10</code>|html}
          db_season_count
      else
        ""
    in
    let seasons_card_html =
      Printf.sprintf
        {html|<div class="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4 sm:p-5 min-w-0"><div class="flex flex-wrap items-start justify-between gap-3 min-w-0"><div class="text-slate-400 font-bold uppercase tracking-widest text-[11px] flex items-center gap-2"><span class="text-base">🗓</span> Seasons</div><div class="flex flex-wrap items-center gap-2 text-[10px] font-mono min-w-0"><span class="px-2 py-0.5 rounded bg-slate-800/60 border border-slate-700/60 text-slate-300 whitespace-nowrap">DB n=%d</span><span class="px-2 py-0.5 rounded bg-slate-800/60 border border-slate-700/60 text-slate-300 whitespace-nowrap">Player n=%d</span></div></div><div class="mt-3 space-y-2 text-slate-200"><div><div class="text-[10px] text-slate-500 font-mono uppercase tracking-widest">DB</div><div class="mt-1 font-mono">%s</div></div><div><div class="text-[10px] text-slate-500 font-mono uppercase tracking-widest">Player</div><div class="mt-1 font-mono">%s</div></div></div>%s</div>|html}
        db_season_count
        player_season_count
    ...
    in
	    ...
	    in
	    Printf.sprintf
	      {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-4 sm:p-6 shadow-lg"><div class="flex items-start justify-between gap-4 mb-4"><h3 class="text-slate-300 font-bold uppercase tracking-wider text-xs flex items-center gap-2"><span class="text-lg">🧾</span> Data Notes</h3><span class="px-2 py-0.5 rounded bg-slate-800/60 border border-slate-700/60 text-[10px] font-mono text-slate-300 whitespace-nowrap">Coverage</span></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3 md:gap-4">%s%s<div class="md:col-span-2 lg:col-span-1 xl:col-span-2">%s</div></div></div>|html}
	      seasons_card_html
	      pbp_card_html
	      draft_card_html
  in
	  let missing_data_html =
	    if profile.season_breakdown = [] && profile.recent_games = [] && profile.all_star_games = [] && profile.draft = None && profile.official_trade_events = [] then
	      {html|<div class="bg-slate-900/50 rounded-xl border border-slate-800/50 p-4 sm:p-6 flex items-center justify-center text-slate-500 text-sm gap-2"><span class="text-xl">🚧</span><div><div class="font-bold">Data Collection in Progress</div><div>Draft info and play-by-play data coming soon.</div></div></div>|html}
	    else
	      ""
	  in
  ...
  let rows =
    if games = [] then
      ...
      |> List.map (fun (g: player_game_stat) ->
          ...
          let opponent_label = if g.is_home then "vs " ^ g.opponent else "@ " ^ g.opponent in
          Printf.sprintf
            {html|<tr class="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors"><td class="px-4 py-3 text-slate-400 text-sm font-mono whitespace-nowrap w-[90px] sm:w-[110px]"><a href="/boxscore/%s" class="hover:text-orange-400 transition-colors">%s</a></td><td class="px-4 py-3 text-white"><div class="flex items-center justify-between gap-3"><span class="player-name truncate">%s</span><div class="flex items-center gap-2 shrink-0">%s%s</div></div></td><td class="px-4 py-3 text-right font-mono text-slate-400 w-[60px] sm:w-[72px] hidden sm:table-cell">%.1f</td><td class="px-4 py-3 text-right font-bold %s w-[60px] sm:w-[72px]">%d</td><td class="px-4 py-3 text-right font-mono w-[60px] sm:w-[72px] %s">%s</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px]">%d</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px] hidden sm:table-cell">%d</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px] hidden sm:table-cell">%d</td><td class="px-4 py-3 text-right text-slate-300 w-[60px] sm:w-[72px] hidden sm:table-cell">%d</td></tr>|html}
            (Uri.pct_encode g.game_id)
            (escape_html g.game_date)
            (escape_html opponent_label)
            margin_badge
...
let team_profile_page (detail: team_full_detail) ~season ~seasons =
  ...
  in
	  let roster_table_inner =
	    Printf.sprintf
	      {html|<table class="roster-table min-w-[820px] w-full text-xs sm:text-sm font-mono table-fixed tabular-nums"><thead class="bg-slate-800/80 text-slate-400 uppercase tracking-wider text-[10px] sm:text-xs whitespace-nowrap"><tr><th class="px-3 py-3 text-left font-sans w-12 whitespace-nowrap">#</th><th class="px-3 py-3 text-left font-sans w-[260px] whitespace-nowrap">Player</th><th class="px-3 py-3 text-right w-[60px] whitespace-nowrap">GP</th><th class="px-3 py-3 text-right text-orange-400 w-[72px] whitespace-nowrap">PTS</th><th class="px-3 py-3 text-right w-[72px] whitespace-nowrap" title="MG: 팀 득실마진(출전시간 가중 평균)">MG</th><th class="px-3 py-3 text-right w-[72px] whitespace-nowrap">REB</th><th class="px-3 py-3 text-right w-[72px] whitespace-nowrap">AST</th><th class="px-3 py-3 text-right w-[72px] whitespace-nowrap">STL</th><th class="px-3 py-3 text-right w-[72px] whitespace-nowrap">BLK</th><th class="px-3 py-2 text-right w-[72px] whitespace-nowrap">TO</th><th class="px-3 py-2 text-right text-orange-400 w-[72px] whitespace-nowrap">EFF</th></tr></thead><tbody>%s</tbody></table>|html}
	      roster_rows
	  in
	  let season_label =
	    if season = "ALL" then
	  ...
	  let game_results_chart =
	    ...
	    match games with
	    | [] ->
	        {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-4 text-sm text-slate-400 shadow-lg">No game results</div>|html}
	    | _ ->
	        let max_abs_margin =
	          games
	          |> List.fold_left
	        ...
	        let bars =
	          ...
	          |> List.map (fun (g: team_game_result) ->
		        ...
		        in
		        Printf.sprintf
		          {html|<div class="bg-slate-900 rounded-xl border border-slate-800 p-4 shadow-lg"><div class="flex items-center justify-between gap-3"><div class="text-[11px] text-slate-400 uppercase tracking-widest font-bold">%s</div><div class="text-xs font-mono text-slate-400 tabular-nums">%d-%d</div></div><div class="mt-2 flex items-center justify-between gap-3 text-[11px] text-slate-500 font-mono tabular-nums"><span>AVG MG %+.1f</span><span>±%d</span></div><div class="mt-3 overflow-x-auto pb-1"><div class="relative h-24 w-max"><div class="absolute left-0 right-0 top-1/2 h-px bg-slate-800/80"></div><div class="flex items-stretch gap-0.5 h-24 w-max pr-1">%s</div></div></div><div class="mt-3 text-[11px] text-slate-500 leading-relaxed">막대: 득실마진(MG=팀-상대). 클릭하면 박스스코어.</div></div>|html}
  ...
  in
	  layout ~title:(t ^ " | WKBL Team Profile")
	    ~content:(Printf.sprintf {html|<div class="space-y-6 sm:space-y-8 animate-fade-in"><div class="bg-slate-900 rounded-xl border border-slate-800 p-5 sm:p-8 shadow-2xl flex flex-col md:flex-row items-center md:items-start gap-6 sm:gap-8"><div class="w-24 h-24 sm:w-32 sm:h-32 bg-slate-800 rounded-xl flex items-center justify-center p-3 sm:p-4 border-2 border-slate-700 shadow-inner">%s</div><div class="text-center md:text-left space-y-4 w-full"><div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3"><h1 class="text-3xl sm:text-4xl font-black text-white">%s</h1><form action="/team/%s" method="get" class="flex flex-col sm:flex-row items-stretch sm:items-center justify-center sm:justify-end gap-2 w-full sm:w-auto"><span class="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Season</span><select name="season" class="bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm focus:border-orange-500 focus:outline-none w-full sm:w-48" onchange="this.form.submit()">%s</select></form></div>%s</div></div><div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8"><div class="space-y-4 lg:col-span-2"><h3 class="text-xl font-bold text-white">Roster</h3><div class="sm:hidden space-y-3">%s</div><details class="sm:hidden bg-slate-900/50 rounded-xl border border-slate-800 p-4"><summary class="cursor-pointer font-bold text-slate-300 select-none">Full table</summary><div class="mt-3 overflow-x-auto">%s</div></details><div class="hidden sm:block bg-slate-900 rounded-xl border border-slate-800 overflow-x-auto shadow-lg">%s</div></div><div class="space-y-6 lg:col-span-1"><h3 class="text-xl font-bold text-white">Game Results</h3>%s<h3 class="text-xl font-bold text-white">Recent Results</h3><div class="bg-slate-900 rounded-xl border border-slate-800 overflow-x-auto overflow-y-hidden shadow-lg"><table class="min-w-[480px] w-full text-xs sm:text-sm font-mono tabular-nums table-fixed"><thead class="bg-slate-800/80 text-slate-400 uppercase tracking-wider text-[10px] sm:text-xs whitespace-nowrap"><tr><th class="px-4 py-3 text-left font-sans w-24">Date</th><th class="px-4 py-3 text-left font-sans">Opponent</th><th class="px-4 py-3 text-center font-sans w-14">Result</th><th class="px-4 py-3 text-right font-sans w-36">Score</th></tr></thead><tbody>%s</tbody></table></div></div></div></div>|html}
