...
let teams_table ~season ~scope (stats: team_stats list) =
  ...
  Printf.sprintf
    {html|<div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 overflow-x-auto overflow-y-hidden"><table class="w-full min-w-[980px] text-xs sm:text-sm font-mono tabular-nums table-fixed" data-sortable id="teams-table-inner"><thead class="bg-slate-100 dark:bg-slate-800/80 sticky top-0 z-10 text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-3 py-2 text-left w-[14%%]">Team</th><th class="px-3 py-2 text-right w-[5%%]" data-sortable data-sort-key="gp">GP</th><th class="px-3 py-2 text-right hidden md:table-cell w-[5%%]" data-sortable data-sort-key="min">%s</th><th class="px-3 py-2 text-right w-[5%%]" data-sortable data-sort-key="pts">PTS</th><th class="px-3 py-2 text-right w-[5%%]" data-sortable data-sort-key="mg" title="MG: 팀 득실마진(PTS - PA)">MG</th><th class="px-3 py-2 text-right hidden md:table-cell w-[5%%]" data-sortable data-sort-key="pa">PA</th><th class="px-3 py-2 text-right hidden sm:table-cell w-[5%%]" data-sortable data-sort-key="reb">REB</th><th class="px-3 py-2 text-right hidden sm:table-cell w-[5%%]" data-sortable data-sort-key="ast">AST</th><th class="px-3 py-2 text-right hidden md:table-cell w-[5%%]" data-sortable data-sort-key="stl">STL</th><th class="px-3 py-2 text-right hidden md:table-cell w-[5%%]" data-sortable data-sort-key="blk">BLK</th><th class="px-3 py-2 text-right hidden md:table-cell w-[5%%]" data-sortable data-sort-key="to">TO</th><th class="px-3 py-2 text-right hidden lg:table-cell w-[6%%]" data-sortable data-sort-key="fg">FG%%</th><th class="px-3 py-2 text-right hidden lg:table-cell w-[6%%]" data-sortable data-sort-key="3p">3P%%</th><th class="px-3 py-2 text-right hidden lg:table-cell w-[6%%]" data-sortable data-sort-key="ft">FT%%</th><th class="px-3 py-2 text-right hidden lg:table-cell w-[6%%]" data-sortable data-sort-key="efg">eFG%%</th><th class="px-3 py-2 text-right hidden lg:table-cell text-emerald-600 dark:text-emerald-400 w-[6%%]" data-sortable data-sort-key="ts" title="True Shooting %%">TS%%</th><th class="px-3 py-2 text-right text-orange-600 dark:text-orange-400 w-[6%%]" data-sortable data-sort-key="eff">EFF</th></tr></thead><tbody>%s</tbody></table></div>|html}
    min_label rows
...
let teams_page ~season ~seasons ~scope ~sort ~include_mismatch stats =
  ...
  layout ~title:"WKBL Teams" ~canonical_path:"/teams"
    ...
    ~content:(Printf.sprintf
      {html|<div class="space-y-6"><div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><h2 class="text-2xl font-bold text-slate-900 dark:text-slate-200">Teams</h2><p class="text-slate-600 dark:text-slate-400 text-sm">Team aggregates by season and scope.</p></div><a class="text-orange-600 dark:text-orange-400 hover:text-orange-700 text-sm" href="/teams">Reset</a></div><form id="teams-filter" class="grid grid-cols-1 md:grid-cols-3 gap-3" hx-get="/teams/table" hx-target="#teams-table" hx-trigger="change"><select name="season" class="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm focus:border-orange-500 focus:outline-none">%s</select><select name="scope" class="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm focus:border-orange-500 focus:outline-none">%s%s</select><select name="sort" class="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm focus:border-orange-500 focus:outline-none">%s%s%s%s%s%s%s%s%s</select><label class="md:col-span-3 flex items-center justify-end gap-2 text-xs text-slate-600 dark:text-slate-400"><input type="checkbox" name="include_mismatch" value="1" %s class="h-4 w-4 rounded border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 accent-orange-500" title="Final score != sum(points) 경기 포함"><span>Mismatch 포함</span></label></form><div id="teams-table" data-skeleton="table" data-skeleton-count="6" data-skeleton-cols="12">%s</div><div class="mt-8"><h3 class="text-lg font-bold text-slate-900 dark:text-slate-200 mb-4">슈팅 효율 비교</h3><div id="teams-shooting-chart" hx-get="/teams/shooting-chart?season=%s" hx-trigger="load" hx-swap="innerHTML" class="htmx-indicator-wrapper"><div class="text-center py-4 text-slate-400"><span class="htmx-indicator">차트 로딩 중...</span></div></div></div></div>|html}
      season_options (scope_option "per_game" "Per Game") (scope_option "totals" "Totals") (sort_option "pts" "PTS") (sort_option "reb" "REB") (sort_option "ast" "AST") (sort_option "stl" "STL") (sort_option "blk" "BLK") (sort_option "eff" "EFF") (sort_option "ts_pct" "TS%") (sort_option "fg3_pct" "3P%") (sort_option "min_total" "MIN") include_checked table season) ()
...
let games_table (games : game_summary list) =
  ...
  in
  Printf.sprintf
    {html|<div class="space-y-3 sm:hidden">%s</div><div class="hidden sm:block bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 overflow-x-auto overflow-y-hidden shadow-2xl"><table class="min-w-[720px] sm:min-w-[860px] w-full text-xs sm:text-sm font-mono tabular-nums"><thead class="bg-slate-100 dark:bg-slate-800/80 sticky top-0 z-10 text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-4 py-3 text-left hidden sm:table-cell">#</th><th class="px-4 py-3 text-left font-sans">Date</th><th class="px-4 py-3 text-left font-sans">Home</th><th class="px-4 py-3 text-center">Score</th><th class="px-4 py-3 text-right font-sans">Away</th><th class="px-4 py-3 text-right font-sans">Action</th></tr></thead><tbody id="games-body">%s</tbody></table></div>|html}
    mobile_cards rows
...
let boxscores_table (games : game_summary list) =
  ...
  let rows =
    ...
    |> List.mapi (fun i (g : game_summary) ->
        ...
        if g.home_score = None then ""
          ...
          Printf.sprintf
            {html|<tr class="border-b border-slate-200 dark:border-slate-800/60 hover:bg-slate-100 dark:bg-slate-800/30 transition-colors"><td class="px-4 py-3 text-slate-600 dark:text-slate-400 font-mono text-sm hidden sm:table-cell">%d</td><td class="px-4 py-3 text-slate-600 dark:text-slate-400 font-mono text-sm w-[90px] sm:w-32 whitespace-nowrap">%s</td><td class="px-4 py-3 font-medium" style="width: max-content; white-space: nowrap;"><span class="inline-flex items-center gap-2" style="white-space: nowrap;">%s<a href="/team/%s" class="team-name hover:text-orange-600 dark:text-orange-400 dark:hover:text-orange-300 transition-colors" style="white-space: nowrap;">%s</a></span></td><td class="px-4 py-3 text-center font-bold text-slate-900 dark:text-slate-200 font-mono">%d</td><td class="px-4 py-3 text-center font-bold text-slate-900 dark:text-slate-200 font-mono">%d</td><td class="px-4 py-3 font-medium text-right" style="width: max-content; white-space: nowrap;"><span class="inline-flex items-center justify-end gap-2" style="white-space: nowrap;"><a href="/team/%s" class="team-name hover:text-orange-600 dark:text-orange-400 dark:hover:text-orange-300 transition-colors" style="white-space: nowrap;">%s</a>%s</span></td><td class="px-4 py-3 text-right font-bold font-mono %s hidden sm:table-cell">%s</td><td class="px-4 py-3 text-right w-[60px] sm:w-20">
  ...
  in
  Printf.sprintf
    {html|<div class="space-y-3 sm:hidden">%s</div><div class="hidden sm:block bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 overflow-x-auto overflow-y-hidden shadow-2xl"><table class="min-w-[720px] sm:min-w-[900px] w-full text-xs sm:text-sm font-mono tabular-nums"><thead class="bg-slate-100 dark:bg-slate-800/80 sticky top-0 z-10 text-slate-600 dark:text-slate-400 text-xs uppercase tracking-wider whitespace-nowrap"><tr><th class="px-4 py-3 text-left hidden sm:table-cell">#</th><th class="px-4 py-3 text-left font-sans">Date</th><th class="px-4 py-3 text-left font-sans">Home</th><th class="px-4 py-3 text-center font-sans">PTS</th><th class="px-4 py-3 text-center font-sans">PTS</th><th class="px-4 py-3 text-right font-sans">Away</th><th class="px-4 py-3 text-right font-sans hidden sm:table-cell">Margin</th><th class="px-4 py-3 text-right font-sans">Link</th></tr></thead><tbody id="boxscores-body">%s</tbody></table></div>|html}
    mobile_cards rows
...
let leader_card_signed title (leaders: leader_entry list) =
  if leaders = [] then ""
    ...
    let others_rows =
      ...
      |> List.mapi (fun i l ->
          let id_badge = if show_id l then Printf.sprintf {html|<span class="ml-2">%s</span>|html} (player_id_badge l.le_player_id) else "" in
          Printf.sprintf {html|<div class="flex items-center justify-between py-2 border-b border-slate-200 dark:border-slate-800/60 last:border-0"><div class="flex items-center gap-3"><span class="text-slate-600 dark:text-slate-400 font-mono text-sm w-4">%d</span>%s<div class="flex flex-col"><div class="text-sm font-medium text-slate-700 dark:text-slate-300"><a href="/player/%s" class="hover:text-orange-600 dark:text-orange-400 transition-colors">%s</a>%s</div><div class="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-400">%s</div></div></div><div class="font-mono font-bold text-slate-600 dark:text-slate-400">%s</div></div>|html} (i + 2) (player_img_tag ~class_name:"w-8 h-8" l.le_player_id l.le_player_name) l.le_player_id (escape_html (normalize_name l.le_player_name)) id_badge (team_logo_tag ~class_name:"w-4 h-4" l.le_team_name) (escape_html (signed l.le_stat_value)))
      |> String.concat "\n"
    in
    Printf.sprintf {html|<div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-lg"><h3 class="text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider text-xs mb-4">%s</h3><div class="flex items-center gap-4 mb-6 pb-6 border-b border-slate-200 dark:border-slate-800"><div class="relative">%s<div class="absolute -top-2 -right-2 bg-orange-500 text-slate-900 dark:text-slate-200 text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center border-2 border-slate-900">1</div></div><div><div class="text-3xl font-black text-slate-900 dark:text-slate-200">%s</div><div class="font-bold text-orange-600 dark:text-orange-400 flex items-center flex-wrap"><a href="/player/%s" class="hover:text-slate-900 dark:hover:text-white dark:text-slate-200 transition-colors">%s</a>%s</div><div class="text-xs text-slate-600 dark:text-slate-400">%s</div></div></div><div class="space-y-1">%s</div></div>|html}
      ...
      top_id_badge
      (team_logo_tag ~class_name:"w-4 h-4" top.le_team_name)
      others_rows
