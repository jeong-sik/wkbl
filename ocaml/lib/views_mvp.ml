...
let mvp_candidate_row (candidate: mvp_candidate) =
  ...
  Printf.sprintf
    {html|<tr class="border-b border-slate-200 dark:border-slate-800/60 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
      <td class="px-3 py-2 text-center whitespace-nowrap">%s</td>
      ...
      %s%s%s%s%s%s
      <td class="px-3 py-2 text-center hidden sm:table-cell whitespace-nowrap"><div class="flex flex-col items-center leading-tight"><span class="text-xs text-slate-500 dark:text-slate-400 font-mono">%s</span><span class="text-slate-500 dark:text-slate-400 text-[10px] font-mono">REC</span></div></td>
      <td class="px-3 py-2 text-right whitespace-nowrap" title="%s"><div class="flex flex-col items-end leading-tight"><span class="font-mono font-bold text-sky-600 dark:text-sky-400">%s</span><span class="text-slate-500 dark:text-slate-400 text-[10px] font-mono">SCORE</span></div></td>
    </tr>|html}
      ...
      {html|<div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 scroll-shadow overflow-y-hidden">
        <table class="w-full min-w-[720px] text-sm table-fixed" aria-label="MVP 레이스 순위">
          ...
          <thead class="bg-slate-100 dark:bg-slate-800/80 sticky top-0 z-10 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
            <tr>
              <th scope="col" class="px-3 py-2 text-center">Rank</th>
              ...
              <th scope="col" class="px-3 py-2 text-right hidden md:table-cell" title="Assists Per Game">APG</th>
              <th scope="col" class="px-3 py-2 text-right hidden lg:table-cell" title="Steals Per Game">SPG</th>
              <th scope="col" class="px-3 py-2 text-right hidden lg:table-cell" title="Blocks Per Game">BPG</th>
              <th scope="col" class="px-3 py-2 text-right hidden sm:table-cell text-orange-600 dark:text-orange-400" title="Efficiency">EFF</th>
              <th scope="col" class="px-3 py-2 text-center hidden sm:table-cell" title="Team Win-Loss Record">Team W-L</th>
              <th scope="col" class="px-3 py-2 text-right text-sky-600 dark:text-sky-400" title="MVP Score = Base + Win Bonus">Score</th>
            </tr>
          </thead>
          <tbody>%s</tbody>
        </table>
