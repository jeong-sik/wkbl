...
let player_profile_page ?(lang=I18n.Ko) ?(leaderboards=None) ?(show_ops=false) (profile: player_profile) ~scope ~(seasons_catalog: season_info list) =
      ...
      let chips_rev, wins, losses, unknown =
        ...
        |> List.fold_left
          (fun (chips, w, l, u) (g: player_game_stat) ->
            ...
            | _ ->
                let chip =
                  {html|<span title="점수 정보 없음" class="inline-flex items-center justify-center w-6 h-6 rounded border bg-slate-100 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700/40 text-[11px] font-mono text-slate-400 dark:text-slate-500 shadow-sm">?</span>|html}
                in
                (chip :: chips, w, l, u + 1))
          ([], 0, 0, 0)
      in
  ...
  in
  let bc_crumbs = [("홈", "/"); ("선수", "/players"); (display_name, "")] in
  let bc_html = render_breadcrumbs bc_crumbs in
  let bc_ld = breadcrumb_json_ld bc_crumbs in
  layout ~lang ~title:(display_name ^ " | WKBL 선수")
    ~canonical_path:canonical
    ~description:seo_desc
    ~og_image:og_card_url
    ~json_ld:(combine_json_ld [json_ld_data; bc_ld])
    ~content:(bc_html ^ Printf.sprintf {html|<div class="space-y-6 sm:space-y-8 animate-fade-in">
      %s
      <!-- 히어로 섹션 개편 -->
...
let player_game_logs_page ?(lang=I18n.Ko) (profile: player_profile) ~(season: string) ~(seasons: season_info list) ~(include_mismatch: bool) (games: player_game_stat list) =
  ...
  in
  let bc_crumbs = [("홈", "/"); ("선수", "/players"); (display_name, player_href p.id); ("경기 로그", "")] in
  let bc_html = render_breadcrumbs bc_crumbs in
  let bc_ld = breadcrumb_json_ld bc_crumbs in
...
let player_splits_page ?(lang=I18n.Ko) (profile: player_profile) ~(season: string) ~(seasons: season_info list) (games: player_game_stat list) =
  ...
  let bc_crumbs = [("홈", "/"); ("선수", "/players"); (display_name, player_href p.id); ("스플릿", "")] in
  let bc_html = render_breadcrumbs bc_crumbs in
  let bc_ld = breadcrumb_json_ld bc_crumbs in
        ...
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            ...
            <form action="%s/splits" method="get" class="flex items-center gap-2">
              <select name="season" aria-label="시즌 선택" class="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm focus:border-orange-500 focus:outline-none" onchange="this.form.submit()">
                %s
              </select>
            </form>
          </div>
...
let player_shot_chart_page ?(lang=I18n.Ko) (profile: player_profile) ~(season: string) ~(seasons: season_info list) (chart: player_shot_chart) =
    ...
    {html|<div class="bg-white dark:bg-slate-900/80 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="text-center">
          ...
          <div class="text-[10px] text-slate-400 dark:text-slate-500">%d/%d</div>
        </div>
        <div class="text-center">
          <div class="text-2xl font-black text-slate-900 dark:text-slate-100 tabular-nums font-mono">%s</div>
  ...
  let bc_crumbs = [("홈", "/"); ("선수", "/players"); (display_name, player_href p.id); ("샷 차트", "")] in
  let bc_html = render_breadcrumbs bc_crumbs in
  let bc_ld = breadcrumb_json_ld bc_crumbs in
  layout ~lang ~title:page_title
